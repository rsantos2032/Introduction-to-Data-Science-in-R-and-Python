...
```{r}
Teams %>% mutate(election = factor(Teams$yearID - Teams$yearID %% 4)) %>% filter(HR == max(HR)) %>% select(teamID, election, HR)
```

The most home runs by a single team was done during the 2016 presidential term. From google this was achieved by the Minnesota Twins in 2019.
...
```{r}
Violations %>% group_by(boro) %>% summarize(prop = mean(grade == "A", na.rm = TRUE))
```
